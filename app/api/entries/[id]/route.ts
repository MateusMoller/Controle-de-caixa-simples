...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!description || !Number.isFinite(amountCents) || amountCents <= 0 || !/^\d{4}-\d{2}-\d{2}$/.test(issueDate) || !/^\d{4}-\d{2}-\d{2}$/.test(dueDate)) {
    return Response.json({ error: "Preencha descrição, valor e datas corretamente." }, { status: 400 });
  }
  const [current] = await getDb().select().from(entries).where(eq(entries.id, Number(id))).limit(1);
  if (!current) return Response.json({ error: "Lançamento não encontrado" }, { status: 404 });
  const paidAmountCents = Math.min(current.paidAmountCents, amountCents);
  const [entry] = await getDb().update(entries).set({
    description,
    ...
    issueDate,
    dueDate,
    paymentMethod: String(payload.paymentMethod ?? "Não informado").trim() || "Não informado",
  }).where(eq(entries.id, Number(id))).returning();
  if (!entry) return Response.json({ error: "Lançamento não encontrado" }, { status: 404 });
  return Response.json({ entry });
}
...
export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!(await requireApiUser())) return Response.json({ error: "Não autorizado." }, { status: 401 });
  const { id } = await context.params;
  const [entry] = await getDb().delete(entries).where(eq(entries.id, Number(id))).returning({ id: entries.id });
  if (!entry) return Response.json({ error: "Lançamento não encontrado" }, { status: 404 });
